...
class KewenController extends CheckController {
	...
	public function music(){
		...
		$Model_music_info=M();
		$sql="SELECT yp.title,yo.id,yo.music_name,yo.music_filename,yo.music_filepath FROM yw_music_info yo,yw_music_group yp WHERE yo.groupid=yp.id";
	...
	public function changeType(){
		...
		if($groupid==0){
			...
			$sql="SELECT yp.title,yo.id,yo.music_name,yo.music_filename,yo.music_filepath FROM yw_music_info yo,yw_music_group yp WHERE yo.groupid=yp.id and yo.groupid='%d'";
	...
	public function updateKechengInfo(){
		...
		echo $grade.'|'.$term.'|'.$version.'|'.$kecheng.'|'.$mobanid.'|'.$types;
	}
	/**
	 * 复制课程
	 */
	public function copyKecheng(){
		$id=I('id/d',0);
		$Model_kecheng_info=M('kecheng_info');
		...
		$Model=M();
		if ($data['mobanid']==2){
			$sql="INSERT INTO yw_guwen(kecheng_id,sentence,description) (SELECT '".$nid."',sentence,description FROM yw_guwen WHERE kecheng_id='".$id."');";
			$Model->execute($sql);
		}
		$sql1="INSERT INTO yw_section_wonderful(kecheng_id,section) (SELECT '".$nid."',section FROM yw_section_wonderful WHERE kecheng_id='".$id."')";
		$Model->execute($sql1);
		$sql2="INSERT INTO yw_step_key(kecheng_id,step,content,tag,time) (SELECT '".$nid."',step,content,tag,time FROM yw_step_key WHERE kecheng_id='".$id."')";
		$Model->execute($sql2);
		$sql3="INSERT INTO yw_text_info(kecheng_id,text_content,words) (SELECT '".$nid."',text_content,words FROM yw_text_info WHERE kecheng_id='".$id."')";
		$Model->execute($sql3);
		$sql4="INSERT INTO yw_word_info(kecheng_id,word,`explain`,sortid) (SELECT '".$nid."',word,`explain`,sortid FROM yw_word_info WHERE kecheng_id='".$id."')";
		$Model->execute($sql4);
	...
	public function fenye(){
		...
		$sql='select count(*) as num from yw_kecheng_info ';
		$sql1='select * from yw_kecheng_info ';
	...
	public function delkecheng(){
		...
		//删除yw_word_info表
		$Model_word_info=M('word_info');
		$Model_word_info->where('kecheng_id="%d"',$id)->delete();
		//删除text_info表
		$Model_text_info=M('text_info');
		$Model_text_info->where('kecheng_id="%d"',$id)->delete();
		//删除step_key表
		$Model_step_key=M('step_key');
		$Model_step_key->where('kecheng_id="%d"',$id)->delete();
		//删除section_wonderful表
		$Model_section_wonderful=M('section_wonderful');
	...
	public function flashdown(){
		...
		$Model_kecheng = M("kecheng_info"); // 实例化User对象
		...
		if($data['mobanid']==1&&$data['isall']==1){
			$moban='现代文';
		}

		if($data['mobanid']==1&&$data['isall']==0){
			$moban = '现代文无重点读';
		}
		if($data['mobanid']==2){
			$moban='古文';
...
// 		$writestring .= 'titleText.text = String("' . $kecheng . '");' . "\r\n			" ;
		$Model_text_info=M('text_info');
		$data_text_info=$Model_text_info->where('kecheng_id="%d"',$kechengid)->find();
		$text = $data_text_info['text_content'];
		$Model_word_info=M('word_info');
		...
		$Model_section_won=M('section_wonderful');
		...
		//特殊字符替换
		$text=str_replace('g', "ɡ", $text);
		$text=str_replace('aliɡn', "align", $text);
			...
			$zhushi = '';
			if($types==1||$types==2){
				$zhushi .= "public var zhushi:String =\"<Chapter><font color='#000000' size='38' face='楷体'><textformat indent='49'><p align='left'>";
			}else{
			...
			if ($types==1){
				...
				foreach ($data_word_info as $v){
					...
					$xuhao='【'.$v['sortid'].'】';
					$replace="<font color='#ff0000' size='14'><a href='event:".$v['explain']."'>[".$v['sortid']."]</a></font>";
					//echo $xuhao.'<br/>';
					//echo $replace.'<br/>';
					$text=str_replace($xuhao, $replace, $text);
				}

				$zhengwen ="public var zhengwen:String=\"<font face='楷体' size='38'><textformat indent='68'>".$text."<a href=''><font color='#ff0000' size='14'> </font></a></textformat></font>\";";
				//$text_quanwen = $text;
				// $pattern = '/<a href=(.*?)a>/';
		  ...
		  //   	$text_ndj = str_replace('<font>','</font>', $text_ndj);
				...
				// $zhengwen ="public var zhengwen:String=\"<font face='楷体' size='34'><p align='center'><b>".$data['kecheng']."</b></p>".$text."<a href=''><font color='#ff0000' size='14'> </font></a></font>\";";
				$zhengwen ="public var zhengwen:String=\"<font face='楷体' size='38'>".$text."<a href=''><font color='#ff0000' size='14'> </font></a></font>\";";
		    	...
		    	$zhengwen = "public var zhengwen:String=\"<font face='宋体' size='30'><p align='left'><textformat indent='60'>".$text_ndj."<a href=''><font color='#ff0000' size='12'> </font></a></textformat></p></font>\";";
		    	$duanju = "public var duanju:String=\"<font face='宋体' size='30'><p align='left'><textformat indent='60'>".$text."<a href=''><font color='#ff0000' size='12'> </font></a></textformat></p></font>\";";
				$text_quanwen = $text_ndj;
				...
				$zhengwen = "public var zhengwen:String=\"<font face='宋体' size='36'>".$text_ndj."<a href=''><font color='#ff0000' size='12'> </font></a></font>\";";
				$duanju = "public var duanju:String=\"<font face='宋体' size='36'>".$text."<a href=''><font color='#ff0000' size='12'> </font></a></font>\";";
				$text_quanwen = $text_ndj;
			...
			foreach($data_section_won as $key=>$v){
				if($types==1||$types==2){
					//现代文下划线放到数组
					$wd = $v['section'];
					$wd=str_replace('g', "ɡ", $wd);
					$xiahuaxianarr.="xiahuaxianarr[".$key."]=\"".trim($wd)."\";\r\n ";
				}else{
					//古诗还是用u标签
			...
			foreach($wordArr as $v){
                ...
                $wdd = $v;
                $wdd=str_replace('g', "ɡ", $wdd);
				$text_quanwen = $this->str_replace_limit(trim($wdd), "<font color='#FF0000'>".trim($wdd)."</font>", $text_quanwen,1);
			}


			if($types==1){
				$wolaidu = "public var wolaidu:String=\"<font face='楷体' size='38'><textformat indent='68'>".$text_quanwen."<a href=''><font color='#ff0000' size='14'> </font></a></textformat></font>\";";
			}else if($types==2){
				$wolaidu = "public var wolaidu:String=\"<font face='楷体' size='38'>".$text_quanwen."<a href=''><font color='#ff0000' size='14'> </font></a></font>\";";
			}else if($types==3){
				$wolaidu = "public var wolaidu:String=\"<font face='宋体' size='30'><p align='left'><textformat indent='60'>".$text_quanwen."<a href=''><font color='#ff0000' size='12'> </font></a></textformat></p></font>\";";
			}else{
				$wolaidu = "public var wolaidu:String=\"<font face='宋体' size='36'>".$text_quanwen."<a href=''><font color='#ff0000' size='12'> </font></a></font>\";";
			}
			...
			foreach ($data_step_key as $key=>$v){
				...
				if($types==1||$types==2){
					//现代文
					$zhongdiandu .= "zhongdianarr[".$key."][0] = \"<Chapter><font color='#000000' size='30' face='楷体'><b><textformat indent='60'>".str_replace("\n", "", trim($v['content']))."</textformat></b></font></Chapter>\";\r\n                ";
					$zhongdiandu .= "zhongdianarr[".$key."][1] = \"<Chapter><font color='#000000' size='30' face='楷体'><b><textformat indent='60'>".str_replace("\n", "", trim($v['tag']))."</textformat></b></font></Chapter>\";\r\n                ";
				}else if($types==3){
			...
			*/
			...
			////新增拼音处理
			$zhengwen = str_replace('（',"（<font face='GB Pinyinok-B' size='38'>",$zhengwen);
			$zhengwen = str_replace('）',"</font>）",$zhengwen);

			$wolaidu = str_replace('（',"（<font face='GB Pinyinok-B' size='38'>",$wolaidu);
			$wolaidu = str_replace('）',"</font>）",$wolaidu);

			$zhongdianarr = str_replace('（',"（<font face='GB Pinyinok-B' size='34'>",$zhongdianarr);
			$zhongdianarr = str_replace('）',"</font>）",$zhongdianarr);

			// $xiahuaxianarr = str_replace('（',"（<font face='GB Pinyinok-B' size='34'>",$xiahuaxianarr);
			// $xiahuaxianarr = str_replace('）',"</font>）",$xiahuaxianarr);
			...
			////新增精彩语句下划线
			$as = str_replace('$$jingcaiyuju$$', $xiahuaxianarr, $as);
...
// 		unlink($zifile);
	...
	*/
	...
	public function kechenginfo(){
		$id=I('id/d',0);
		$Model_kecheng=M('kecheng_info');
		$data_kecheng=$Model_kecheng->where('id="%d"',$id)->field('mobanid,filepath')->find();
		$this->assign('mobanid',$data_kecheng['mobanid']);
		$this->assign('filepath',$data_kecheng['filepath']);
		$Model_word_info=M('word_info');
