<?php
	class FileUpload {
		...
		private $isthumb=false;	//是否生成缩略图,true生成,false不生成
		...
		private function copyFile(){
			if(!$this->errorNum){
				...
				$filepathImg=$filepath.$this->newFileName;
				
				if(@move_uploaded_file($this->tmpFileName, $filepathImg))	{
					if ($this->isthumb){
						//如果为真，生成缩略图
						$thumbImg=$filepath.substr($this->newFileName,0,strrpos($this->newFileName, '.')).'_thumb.'.$this->fileType;
						$img = new \Think\Image();
						$img->open($filepathImg);
						$img->thumb(150, 150);
						$img->save($thumbImg);						
					}
					return true;
